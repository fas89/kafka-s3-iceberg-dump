 ...
 * }</pre>
 *
 * Field order must match {@code FlinkIngestJob.SCHEMA}:
...
public final class JsonToRowData implements MapFunction<String, RowData> {
  ...
  public RowData map(String json) throws Exception {
    ...
    row.setField(0, str(text(node, "id")));
    row.setField(1, str(text(node, "event_type")));
    row.setField(2, str(text(node, "user_id")));
  ...
  /** Wraps a string as Flink {@link StringData}, preserving SQL null. */
